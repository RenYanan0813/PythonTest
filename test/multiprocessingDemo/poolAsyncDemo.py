...
实例方法：
    ...
    map_async(func, iterable[, chunksize=None])：
        map_async与map的关系同apply与apply_async
...
方法apply_async()和map_async()的返回值是AsyncResul的实例obj。实例具有以下方法：
    ...
    ready()：如果调用完成，返回True
    successful()：如果调用完成且没有引发异常，返回True，
        如果在结果就绪之前调用此方法，引发异常
...
def testPool2(i):
    time.sleep(1)
    print("这是name = %s 第... %s ...个进程！" % (os.getpid(), i))
...
if __name__ == "__main__":
    ...
    for i in range(48):
        print("......%s....", i)
            ...
            # （2）每次执行8个子进程，等一个子进程执行完后，
            #     立马启动新的子进程。（相对父进程不阻塞）
            #     apply_async为异步进程池写法。
            #     异步指的是启动子进程的过程，
            #     与父进程本身的执行（print）是异步的，
            #     而For循环中往进程池添加子进程的过程，
            #     与父进程本身的执行却是同步的。
        # 维持执行的进程总数为8，当一个进程执行完后启动一个新进程.
        pool.apply_async(testPool2, args=(i,))
    print("testPool2")
    pool.close()
    pool.join()
