...
实例方法：
    ...
    run()：进程启动时运行的方法，正是它去调用target指定的函数，
        我们自定义类的类中一定要实现该方法
    terminate()：强制终止进程p，不会进行任何清理操作，如果p创建了子进程，
...
def run_proc(name):
    print('Run child procedd %s (%s)'% (name, os.getpid()))

if __name__ == '__main__':
    #注意：在windows中Process()必须放到if __name__ == ‘__main__’:下
    print('Parent process %s.' % os.getpid())
    p = Process(target = run_proc, args = ('test', ))
    print('chrild process will start.')
    p.start()
    p.join() #p.join只能join住start开启的进程，而不能join住run开启的进程
    print("chrild process will end......")
...
'''输出：
    Parent process 3776.
    chrild process will start.
    child process end.
    Run child procedd test (12580)
    child process end.
'''