...
def listener_cpu(hsn, usn, psw, out, svr_name, sleep_time, ste_time):
    ...
    try:
        ...
        while(flag):
            ...
            print >> fp, "*********************%s --- %s 服务器的---- cpu 使用情况 **********************"%(date, hsn)
            stdin, stdout, stderr = os.system("top -bi -n 1 -d 0.02")
            ...
            fp1 = open(svr_out, 'a')
            print >> fp1, "*********************%s --- %s 服务器的---- 某些服务使用 cpu 情况 **********************" % (date, hsn)
            print >> fp1, "...   PID USER      PR  NI    VIRT    RES    SHR S  %CPU %MEM     TIME+ COMMAND \n"
            for svr in range(len(server_name)):
                comd = "top -bi -n 1 -d 0.02 | grep %s"%(server_name[svr])
                stdin3, stdout3, stderr3 = os.system(comd)
                for line in stdout3:
                    print >> fp1,'... ' + line.strip('\n')
                print(stderr3)
            print >> fp1,"\n\n"


            fp2 = open(mem_out, 'a')
            print >> fp2, "*********************%s --- %s 服务器的----  mem 使用情况 **********************"%(date, hsn)
            stdin1, stdout1, stderr1 = os.system("free -h")
            print >> fp2, "..........                    total        used        free      shared  buff/cache   available \n"
            i = 0
            for line in stdout1:
                i += 1
                if i != 1:
                    print >> fp2, '.......... ' + line.strip('\n')
            print >> fp2, "\n\n"

            fp3 = open(disk_out, 'a')
            print >> fp3, "*********************%s --- %s 服务器的----  磁盘 使用情况 **********************"%(date, hsn)
            stdin2, stdout2, stderr2 = os.system("df -h")
            for line in stdout2:
                print >> fp3, '... ' + line.strip('\n')
            print >> fp3,"\n\n"
