...
def coppy_server(version_ip):
    ...
    time.sleep(5)
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    stdin, stdout, stderr = ssh.exec_command('cd %s/tmp; cp -r qrysvr qrysvrA; mv qrysvr qrysvrB; cp -r qsvr qsvrA; mv qsvr qsvrB; cp -r tra_syncsvr tra_syncsvrA; mv tra_syncsvr tra_syncsvrB'%tar_path)
    time.sleep(5)
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ...
    if 'tra_syncsvr-A.cfg' in new_conf_name:
        with open('tra_syncsvr-A.cfg','r+') as fr:
            ...
            for line in data:
                if '"grp_cfg"' in line :
                    num = data.index(line)
                    data[num] = '"grp_cfg" : "/nfs/conf/group_tra.cfg",\n'
                if '"name"' in line:
                    num = data.index(line)
                    data[num] = '"name" : "./log/tra_syncsvr-A",\n'
    ...
    if 'tra_syncsvr-B.cfg' in new_conf_name:
        with open('tra_syncsvr-B.cfg', 'r+') as fr:
            ...
            for line in data:
                ...
                if '"name"' in line:
                    num = data.index(line)
                    data[num] = '"name" : "./log/tra_syncsvr-B",\n'
    ...
    if 'qsvrB' in ser_list:
        ...
        with open('qsvr-B.cfg','r+') as fk:
            ...
            for line in data:
                ...
                if '"grp_cfg" :' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"grp_cfg" : "/nfs/conf/group_tra.cfg",\n'
    ...
    if 'initsvr_tra' in ser_list:
        with open('initsvr.cfg','r+')as finit:
            ...
            for line in data:
                if '"name"' in line:
                    line_num = data.index(line)
                    data[line_num] = '"name":"./log/initsvr",\n'
...
def modefy_tips_initsvr_tra(ser_list,conf_name):
    if 'tips_initsvr_tra' in ser_list:
        ...
        for line in data:
            if '"name":' in line:
                line_num = data.index(line)
                data[line_num] = '"name":"./log/tips_init",\n'
            if '"comm_db_cfg":' in line:
                line_num = data.index(line)
                data[line_num + 1] = '"username":"par_user",\n'
                data[line_num + 2] = '"password":"oracle123!",\n'
    ...
    if 'bussvrA' in ser_list:
        ...
        with open('bussvr-A.cfg','r+') as fb:
            ...
            for line in data:
                if '"name":' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"name":"./log/bussvr-A",\n'
                if '"listen_port"' in line:
                    lin_num = data.index(line)
                ...
                if '"grp_cfg" :' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"grp_cfg" : "/nfs/tra_conf/group_tra.cfg",\n'
    ...
    if 'bussvrB' in ser_list:
        ...
        with open('bussvr-B.cfg','r+') as fb:
            ...
            for line in data:
                if '"name":' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"name":"./log/bussvr-B",\n'
                if '"listen_port"' in line:
                    lin_num = data.index(line)
            ...
            if 'matchsvrA' in ser_list:
                ...
                with open('matchsvr-A.cfg','r+') as fm:
                    ...
                    for line in data:
                        if '"name"' in line :
                            lin_num = data.index(line)
                            data[lin_num] = '"name":"./log/matchsvr-A",\n'
                        if '"listen_port"' in line:
                            lin_num = data.index(line)
            ...
            if 'matchsvrB' in ser_list:
                ...
                with open('matchsvr-B.cfg','r+') as fm:
                    ...
                    for line in data:
                        if '"name"' in line :
                            lin_num = data.index(line)
                            data[lin_num] = '"name":"./log/matchsvr-B",\n'
                        if '"listen_port"' in line:
                            lin_num = data.index(line)
            ...
            if 'qrysvrA' in ser_list:
                ...
                with open('qrysvr-A.cfg', 'r+') as fm:
                    ...
                    for line in data:
                        if '"name"' in line:
                            lin_num = data.index(line)
                            data[lin_num] = '"name":"./log/qrysvr-A",\n'
                        if '"listen_port"' in line:
                            lin_num = data.index(line)
            ...
            if 'qrysvrB' in ser_list:
                ...
                with open('qrysvr-B.cfg', 'r+') as fm:
                    ...
                    for line in data:
                        if '"name"' in line:
                            lin_num = data.index(line)
                            data[lin_num] = '"name":"./log/qrysvr-B",\n'
                        if '"listen_port"' in line:
                            lin_num = data.index(line)
            ...
            if 'bridgesvr_traA' in ser_list:
                ...
                with open('bridgesvr_tra-A.cfg', 'r+') as fm:
                    ...
                    for line in data:
                        if '"name"' in line:
                            lin_num = data.index(line)
                            data[lin_num] = '"name":"./log/bridgesvr_tra-A",\n'
                        if '"listen_port"' in line:
                            lin_num = data.index(line)
            ...
            if 'bridgesvr_traB' in ser_list:
                ...
                with open('bridgesvr_tra-B.cfg', 'r+') as fm:
                    ...
                    for line in data:
                        if '"name"' in line:
                            lin_num = data.index(line)
                            data[lin_num] = '"name":"./log/bridgesvr_tra-B",\n'
                        if '"listen_port"' in line:
                            lin_num = data.index(line)
    ...
    if 'keepsvr-A.cfg' in conf_name:
        with open('keepsvr-A.cfg', 'r+') as fm:
            ...
            for line in data:
                if '"name"' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"name":"./log/keepsvr-A",\n'
                if '"grp_cfg" :' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"grp_cfg" : "/nfs/conf/group_tra.cfg",\n'
    ...
    if 'keepsvr-B.cfg' in conf_name:
        with open('keepsvr-B.cfg', 'r+') as fm:
            ...
            for line in data:
                if '"name"' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"name":"./log/keepsvr-B",\n'
                if '"grp_cfg" :' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"grp_cfg" : "/nfs/conf/group_tra.cfg",\n'
    ...
    if 'quot_mem2dbsvr.cfg' in conf_name:
        with open('quot_mem2dbsvr.cfg', 'r+') as fm:
            ...
            for line in data:
                if '"name"' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"name":"./log/quot_mem2dbsvr",\n'
                if '"grp_cfg" :' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"grp_cfg" : "/nfs/conf/group_tra.cfg",\n'
    ...
    if 'trans_mem2dbsvr.cfg' in conf_name:
        with open('trans_mem2dbsvr.cfg', 'r+') as fm:
            ...
            for line in data:
                if '"name"' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"name":"./log/trans_mem2dbsvr",\n'
                if '"grp_cfg" :' in line:
                    lin_num = data.index(line)
                    data[lin_num] = '"grp_cfg" : "/nfs/conf/group_tra.cfg",\n'
...
if __name__ == '__main__':
    big_list = read_excel('tra_conf198-201real.xls')
    main(big_list)
