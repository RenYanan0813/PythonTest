...
def coppy_3_directories(ipp,unm,passd,old_version_path):  ##复制结算行挡板等文件。
    ...
    if 'acsvr_bank' in tem:
        print('acsvr_bank in')
        ssh.exec_command('cd %s/%s; cp -r acsvr_bank ../install' % (back_dir, old_install_name))
        time.sleep(5)
        print('acsvr_bank 复制成功 。')
    else:
        print('not found acsvr_bank')
    ssh.close()
...
def modefy_rescur(ipp,unm,passd,ser_list,conf_name,old_version_path):
    if 'rescsvr' in ser_list:
        with open('rescsvr.cfg','r+') as fr:
            data = fr.readlines()
            for line in data:
                if '"grp_cfg"' in line :
                    num = data.index(line)
            ...
            with open('rescsvr.cfg','w+') as fr2:
                for line in data:
                    fr2.write(line)
                print('*********rescsvr.cfg ok ! *******')
    if 'syncsvr-A.cfg' in conf_name:
        with open('syncsvr-A.cfg','r+') as fr:
            data = fr.readlines()
            for line in data:
                if '"grp_cfg"' in line :
                    num = data.index(line)
                ...
                if '"idx"' in line:
                    num = data.index(line)
                    data[num] = '"idx" : "/home/reg1/install/ctrlsvr/data/flowA.idx",\n'
                    data[num +1 ] = '"con" : "/home/reg1/install/ctrlsvr/data/flowA.con",\n'
            with open('syncsvr-A.cfg','w+') as fr2:
                for line in data:
                    fr2.write(line)
                print('*********syncsvr-A.cfg ok ! *******')
    if 'syncsvr-B.cfg' in conf_name:
        with open('syncsvr-B.cfg', 'r+') as fr:
            data = fr.readlines()
            for line in data:
                if '"grp_cfg"' in line:
                    num = data.index(line)
                ...
                if '"idx"' in line:
                    num = data.index(line)
                    data[num] = '"idx" : "/home/reg1/install/ctrlsvr/data/flowB.idx",\n'
                    data[num +1 ] = '"con" : "/home/reg1/install/ctrlsvr/data/flowB.con",\n'
            with open('syncsvr-B.cfg', 'w+') as fr2:
                for line in data:
                    fr2.write(line)
                print('*********syncsvr-B.cfg ok ! *******')
    if 'rescsvr_db' in ser_list:
        with open('rescsvr_db.cfg', 'r+') as fr:
            data = fr.readlines()
            for line in data:
                if '"grp_cfg"' in line:
                    num = data.index(line)
                ...
                if '"user_nm"' in line :
                    ...
                    data[num + 1] = '"user_pwd"  : "Oracle123!",\n'
                    data[num + 2] = '"orc_inst"  : "REG3_REG",\n'
            with open('rescsvr_db.cfg', 'w+') as fr2:
                for line in data:
                    fr2.write(line)
                print('*********rescsvr_db.cfg ok ! *******')

def sync_com(ipp,unm,passd,ser_list,conf_name,old_version_path):
    if 'syncsvr-A.cfg' in conf_name:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(hostname=ipp, username=unm, password=passd)
        ...
        old_install_name = 'install_%s' % date_str
        back_dir = os.path.split(old_version_path)[0]
        ssh.exec_command('cd %s/%s/syncsvr_reg/conf; cp group_sync.cfg /home/reg1/install/syncsvr_reg/conf'%(back_dir, old_install_name))
        print('**************** A  cp group_sync.cfg sucessfully ********************** ')
        time.sleep(2)
        ssh.close()
    if 'syncsvr-B.cfg' in conf_name:
        ssh = paramiko.SSHClient()
        ...
        back_dir = os.path.split(old_version_path)[0]
        ssh.exec_command('cd %s/%s/syncsvr_reg/conf; cp group_sync.cfg /home/reg1/install/syncsvr_reg/conf'%(back_dir,old_install_name))
        print('**************** B cp group_sync.cfg sucessfully ********************** ')
        time.sleep(2)
        ssh.close()
            ...
            with open('acsvr_wh.cfg','r+') as fw:
                ...
                for line in data:
                    if '"server_ip"' in line:
                        nu = data.index(line)
                        data[nu] = '        "server_ip" : "180.2.32.20",\n'
                        data[nu+1] = '        "server_port" : 7777,\n'
            with open('acsvr_wh.cfg', 'w+') as f2:
                for lines in data:
                    f2.write(lines)
            print('acsvr_wh 修改 ip port 成功')
        else:
...
def rm_noserver(ipp,unm,passd,ser_list,conf_name,old_version_path):
    ...
    ac_group = ['acsvr_wm','acsvr_bank','acsvr_acct']
...
def main(total):
    ...
    for sercell in big_list:
        ...
        modefy_rescur(ipp,unm,passd,ser_list,conf_name,old_version_path)
        sync_com(ipp, unm, passd, ser_list, conf_name, old_version_path)
        upload_confed_files(ipp,unm,passd,ser_list,conf_name,old_version_path)
...
if __name__ == '__main__':
    big_list = read_excel('reg_conf22920real.xls')
    main(big_list)
